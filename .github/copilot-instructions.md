...
- Always run markdown linting on documentation changes:

  ```bash
  npm install -g markdownlint-cli
  markdownlint '**/*.md' --ignore 'node_modules' --ignore 'src' --ignore '**/bin/**' --ignore '**/obj/**' --ignore '**/lib/**' --ignore '**/GenAiLab/README.md' --disable MD013 MD033 MD041
  ```

- Always run .NET formatting on code changes:

  ```bash
  dotnet format [solution-or-project-path] --verify-no-changes  # Check for formatting issues
  dotnet format [solution-or-project-path]  # Fix formatting issues
  ```
...
- **ALWAYS run through complete scenarios** after making changes:
  ...
  3. **Template Validation**: Test creating new projects with templates:
     ...
     dotnet build /tmp/test-app/TestApp.csproj --configuration Release
     dotnet build /tmp/test-mcp/TestMcp.csproj --configuration Release
     ```

  4. **Repository-wide Validation**: Test all projects can be built:

     ```bash
     # Build all solutions/projects in CI
     for solution in "Part 2 - Project Creation/GenAiLab/GenAiLab.sln" "Part 6 - Deployment/GenAiLab/GenAiLab.sln"; do
       dotnet build "$solution" --configuration Release
     done
     
     for project in "Part 7 - MCP Server Basics/MyMcpServer/MyMcpServer.csproj" "Part 8 - Enhanced MCP Server/ContosoOrdersMcpServer/ContosoOrdersMcpServer.csproj" "TestMcpServer/TestMcpServer.csproj"; do
