# Part 9: Adding AI to an Existing App with eShopLite

> **⏱️ Estimated Time:** 45-60 minutes
>
> **Prerequisites**: Complete [Part 8: Agent Framework Essentials](../Part%2008%20-%20Agent%20Framework%20Basics/README.md). [Part 3: Add RAG](../Part%2003%20-%20Add%20RAG/README.md) and [Part 5: MCP Server Basics](../Part%2005%20-%20MCP%20Server%20Basics/README.md) are assumed background.
...
## Why this comes after Part 8

Every technique in this part is something you have already seen in isolation:

| Capability area | Builds on |
| --- | --- |
| Product discovery | Embeddings and semantic search ([Part 3](../Part%2003%20-%20Add%20RAG/README.md)) |
| Operations assistant | Local models and provider choice (looked at in detail in [Part 10](../Part%2010%20-%20Choosing%20Providers%20and%20Services/README.md)) |
| Business reporting | Grounded prompting over your own data ([Part 2](../Part%2002%20-%20Build%20Chat%20App/README.md)) |
| App capabilities as tools | MCP servers ([Part 5](../Part%2005%20-%20MCP%20Server%20Basics/README.md)) |
| Agents collaborating | `AIAgent`, specialist agents, orchestration ([Part 8](../Part%2008%20-%20Agent%20Framework%20Basics/README.md)) |
...
This is the **cloud-for-users, local-for-operations** split, and it is a concrete payoff from writing against `IChatClient` instead of a specific provider: running one feature locally and another in the cloud is a wiring change, not a rewrite. Part 10 goes through the provider options in detail.
...
**Technique:** expose a small set of reviewed capabilities as MCP tools — the same server shape you built in [Part 5](../Part%2005%20-%20MCP%20Server%20Basics/README.md), now pointed at a real app.
...
**Problem:** "sales of rain jackets dropped this week — is that demand or a bug?" spans the catalog, the telemetry, and the business data. One agent holding every tool for all three becomes unreliable, exactly as described in Part 8.
...
The orchestrator routes the parts of the question, each specialist answers only from its own tools, and the findings are combined. Because each agent stays small, each one stays debuggable — and each one is just the single agent you already built in Part 8.
...
Nothing in `OperationsAssistant.cs` changes. That is the provider abstraction paying off in a scenario that actually motivates it: log data never leaves the machine, and there is no per-token cost for reading telemetry.
