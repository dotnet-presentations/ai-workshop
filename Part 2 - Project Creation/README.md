...
Your application uses **Azure AI Foundry** (Azure OpenAI models) as its AI provider. You'll need an Azure OpenAI resource with the `gpt-5-mini` (chat) and `text-embedding-3-small` (embeddings) models deployed.
...
1. Deploy (or obtain access to) an Azure OpenAI resource with:
   - `gpt-5-mini` deployed for chat completions
...
1. Verify your Azure OpenAI (Azure AI Foundry) resource has `gpt-5-mini` and `text-embedding-3-small` deployed
