...
| `Microsoft.McpServer.ProjectTemplates` 1.2.1 and `ModelContextProtocol` 2.x | Parts 5 and 6 MCP servers | The `mcpserver` template is included in .NET 10 SDK 10.0.9+ (or `Microsoft.McpServer.ProjectTemplates` 1.2.1) and ships with `ModelContextProtocol` 2.x, which is sufficient for the workshop exercises without requiring a manual package upgrade after scaffolding. | Re-scaffold Part 5 with the template, compare packaging and metadata, then build and exercise both stdio servers with a compatible client. Keep the registry schema tied to the template and registry specification rather than the SDK protocol version. |
