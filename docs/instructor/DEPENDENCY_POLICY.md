...
| --- | --- | --- | --- |
| `Microsoft.Bcl.Memory` 10.0.12 | Part 3 MEDI checkpoint | Promotes the transitive dependency brought in by tokenizer packages above a version with a high-severity advisory. | A restore without the direct reference resolves a non-vulnerable version. A newer safe patch may replace 10.0.10 after affected builds remain warning-free. |
| `Microsoft.Bcl.Memory` 10.0.12 | Part 11 GenAiLab web app | Promotes the transitive dependency brought in by tokenizer packages above a version with a high-severity advisory. | A restore without the direct reference resolves a non-vulnerable version. A newer safe patch may replace 10.0.12 after affected builds remain warning-free. |
| `SQLitePCLRaw.bundle_e_sqlite3` 3.0.5 | Part 3 MEDI checkpoint; both Part 9 Products projects; Part 4 Docker-free instructions | Promotes the native SQLite bundle above the vulnerable transitive version used by SqliteVec or the local template path. | The parent packages resolve a non-vulnerable native bundle without the override. A newer safe patch is allowed after local SQLite and vector-search behavior is tested. |
| `Microsoft.OpenApi` 2.12.2 | Both Part 9 Products projects | Originally added as a security override. It is also bounded by `Microsoft.AspNetCore.OpenApi` 10.0.x, which requires `Microsoft.OpenApi` below 3.0. Version 2.12.2 is the latest compatible 2.x release; updating only this package to 3.x produces `NU1608` and source-generator compile errors. | The selected `Microsoft.AspNetCore.OpenApi` version supports 3.x, or its transitive dependency is safe enough to remove the direct reference. Update the start and completed snapshots together. |
...
| --- | --- | --- | --- |
| Aspire 13.4.6 packages and AppHost SDK | Parts 4, 10, and 11 GenAiLab progression | This is the version emitted by the current AI templates and used throughout the workshop. Keep the SDK and Aspire packages aligned as one template-bound set; do not add an attendee upgrade step unless the complete template progression is intentionally rebaselined. | Update the Part 4 commands, Part 10 provider commands, Part 11 snapshot, and instructor template notes together. Test local orchestration and deployment configuration. |
| Aspire AppHost SDK 13.4.6 | Part 9 `eShopLite-start` and `eShopLite` | This is the baseline inherited by the existing-app scenario and is aligned with the rest of the workshop. The start and answer projects must remain equivalent before AI changes are applied. | Update both AppHost projects together, confirm the starting application still matches the lab, and run the completed AI flows. |
| `mcpserver` template and `ModelContextProtocol` 2.x | Parts 5 and 6 MCP servers | The `mcpserver` project template is included in the .NET 10 SDK (10.0.9+) and emits `ModelContextProtocol` 2.x, which is sufficient for the workshop exercises without requiring a manual package upgrade after scaffolding. | Re-scaffold Part 5 with the SDK template, compare packaging and metadata, then build and exercise both stdio servers with a compatible client. Keep the registry schema tied to the template and registry specification rather than the SDK protocol version. |
