...
| `Microsoft.McpServer.ProjectTemplates` 1.2.1 and `ModelContextProtocol` 2.2.0 | Parts 5 and 6 MCP servers | Template 1.2.1 is the latest template baseline and emits SDK 1.2.0. Part 5 therefore adds an explicit package upgrade after scaffolding; both active server snapshots use SDK 2.2.0 and the template's current `2025-10-17` registry metadata schema. | Re-scaffold Part 5 with the exact template version, apply the documented SDK upgrade, compare packaging and metadata, then build and exercise both stdio servers with a compatible client. Keep the registry schema tied to the template and registry specification rather than the SDK protocol version. |
